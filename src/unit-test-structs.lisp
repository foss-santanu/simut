...
(defmacro create-fixture (fixture-name &rest forms)
  `(setf (symbol-function ',fixture-name)
        #'(lambda () (format t "Executing fixture: ~S ...~%" ',fixture-name)
                     ,@forms)))
...
;; print-test-result: prints results after running the tests
;; Arguments: test-name - name of the unit test (optional)
...
(defun print-test-result (&key test-name suite-name (summary-p t))
    ;; print-result: prints a single test result entry
    (labels ((print-result (report-item total-success total-failure)
            ...
            (let ((total-success 0) (total-failure 0) (report-item))
                 (cond
                      ;; unit test name supplied
                      ;; if test suite provided then report item is test suite
                      (test-name (if suite-name (setf report-item suite-name)
                      ...
                      ;; test suite supplied - prints results from running the test suite
                      ((and suite-name (not test-name))
                                 (setf report-item suite-name)
                                 (format t "Result for test case: ~A~%" suite-name)
                                 (multiple-value-setq (total-success total-failure)
                                     (print-result report-item total-success total-failure)))
                      ;; prints results from running all the tests
                      ((and (not suite-name) (not test-name))
...
(defun run-test (test-name &key in-suite set-up tear-down (print-result-p t))
    ...
    (when print-result-p (print-test-result :test-name test-name :suite-name in-suite)))
    ...
    (let ((suite (gethash suite-name *global-test-suite*)) (test-name))
         ...
         (with-success-failure-handlers (test-name :suite-name suite-name)
             (maphash #'(lambda (test-nm test-fn) (setf test-name test-nm) (exec-test test-fn))
                      suite))
         (when tear-down (funcall tear-down))
         (when print-result-p (print-test-result :suite-name suite-name))))
    ...
    (let ((suite-name) (test-name))
         ...
         (with-success-failure-handlers (test-name :suite-name suite-name)
             (maphash #'(lambda (suite-nm suite)
                      ...
                      *global-test-suite*))
         (when tear-down (funcall tear-down))
         (when print-result-p (print-test-result))))
